...
function modifyDisplayEvents(chinaEvents) {
  ...
  function formatChinaTimeISO(currentT) {
    //接受iso格式的日期，设置成中国时间后，转成ISO格式
    let tzoffset = (new Date()).getTimezoneOffset() * 60000; //offset in milliseconds
    let localISOTime = (new Date(currentT - tzoffset)).toISOString().slice(0, -1);
    return localISOTime
  } 
